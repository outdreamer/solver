I. To do list
	...
	- identify useful structures like 'ways to reduce a solution space' like 'specifying the problem (without contradicting the target problem, like by maintaining the problem type)' or 'specifying relevant graphs to a degree/structure that reduces the solution space of problems related to those graphs' or 'identifying variables/spectrums/structures connecting graphs with one possible solution (which is not correct for the target problem) to graphs with additional possible solutions (which could be correct for the target problem) and identifying additivity of variables of these graphs to generate one graph type from another'
