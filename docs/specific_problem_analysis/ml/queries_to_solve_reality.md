I. To do list
	...
	- other queries
		...
		- identify useful networks like 'networks of variables organized by causality/similarity/relevance/interactivity/meaning' (like organizing all the components of polynomials to be adjacent and all the linear variables to be adjacent or all the specifying variables to be adjacent to general variables like function types or maximally different functions or other function bases) so that the 'sets of relevant variables for an intent' are more predictable and queries like 'combine a variable from each variable type or opposite/similar variable types' can be trivially run on the network
