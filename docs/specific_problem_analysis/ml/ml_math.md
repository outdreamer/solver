...
1. Underflow & Overflow
  - the landscape has cliffs and plateaus that are exponentially steep or flat.
    Functions like exp(x), the softmax, log-likelihoods, etc., grow or shrink exponentially.
    Picture a 1-D terrain where: One side shoots up like a vertical cliff (large positive x)
    The other side plummets into an almost perfectly flat valley (large negative x)
    A floating-point system has a limited “altitude range.”
    If you climb too high, readings become ∞ (overflow).
    If you go too low, the valley floor collapses to 0 (underflow).
  - cause: Exponential curves expand or contract distances so aggressively that numerical “altitude bands” cannot accommodate the true shape.
2. Rounding Errors Accumulate
  - many tiny directional steps accumulate drift, like walking around a sphere but measuring angles in rough increments.
    Floating-point numbers create a grid over space.
    Every time you do a computation, you snap to the nearest grid point.
    If you: walk 1 million small steps, each with a tiny angular or position error, you don’t return to the exact start — you drift.
  - cause: High-dimensional trajectories (gradients, iterative updates) amplify tiny directional errors. Over thousands of steps, paths diverge from “true mathematical paths.”
3. Catastrophic Cancellation
  - subtracting nearly parallel vectors erases the meaningful direction.
    Consider two long vectors pointing almost the same way: In exact math, their difference is a tiny vector showing their subtle geometric misalignment.
    In floating-point, their large components cancel imprecisely, and the tiny difference becomes noise.
    When vectors are nearly parallel: They occupy nearly the same “line” in space.
    Subtraction tries to isolate the perpendicular “difference.”
    But the perpendicular component is orders of magnitude smaller than the parallel components.
    Floating-point cannot “resolve” that narrow wedge.
    This destroys important curvature or gradient information.
4. Ill-Conditioning & Condition Numbers
  - transformations that stretch space unevenly distort errors like funhouse mirrors.
    An ill-conditioned matrix transformation stretches space in some directions and squashes it in others.
    Imagine a circle transformed into a very long, skinny ellipse: A tiny motion in the stretched direction becomes large.
    A tiny error in the squashed direction becomes invisible.
    If matrix A sends orthogonal directions to vectors with wildly different lengths, then solving Ax=b attempts to invert this distortion.
  ...
  - cause: Ill-conditioning = some directions are razor-thin and others are huge → inverses multiply errors.
5. Gradients in Deep Networks Vanish or Explode
  - repeatedly stretching/contracting space along a chain of transformations makes some paths disappear and others blow up.
    Layers perform transformations: Each multiplication tilts, stretches, or compresses space.
    If most layers compress slightly (<1 singular value), the overall effect is exponential shrinking → vanishing gradients.
    If they stretch slightly (>1 singular value), the effect compounds → exploding gradients.
  - cause: Repeated linear transformations act like multiplying many ellipses.
    The combined ellipse either collapses along some directions or expands along others.
    That is exactly what “vanish” and “explode” mean geometrically.
6. Non-Convexity & Saddle Points
  - most critical points in high-dimensional terrain are saddles, not valleys — they are stable in some directions and unstable in others.
    A saddle is like sitting on a horse: One direction curves upward (looks like a minimum)
    Another curves downward (looks like a maximum)
    In high dimensions, almost every stationary point behaves like this.
    Gradient descent: sees almost-zero slope (gets confused)
    tiny numeric noise from floating-point may kick it off the saddle unpredictably
    second-order methods see contradictory curvature signs and break down
    - cause: Curvature has mixed signs → numerical steps can become unstable or ambiguous.
7. Softmax Instability
  - exponentials cause different logits to occupy wildly separated “height scales.”
    But if one coordinate is much larger: That direction stretches into the exponential cliff.
    All other directions collapse to the valley floor (zero).
    Subtracting max(x) simply recenters the geometry so the cliff height fits in finite altitude range.
  - cause: Exponentials make the scale of height differences too extreme for fixed-resolution arithmetic.
8. Hessian & Second-Order Instability
  - curvature is extremely irregular in deep networks; the Hessian often has huge eigenvalue spreads.
    The Hessian encodes: bendiness, flatness, twisting directions in high-dimensional space
    If it has: a few huge eigenvalues (very steep ravines), many tiny eigenvalues (flat plateaus), some negative eigenvalues (downhill directions)
    Then Newton’s method tries to invert that wildly uneven curvature map.
    Inverting this is like trying to “unscramble” a shape that is simultaneously: almost flat, extremely curved, flipped in some directions
    Numerically, this explodes.
  - cause: The curvature landscape is too irregular to invert stably.
...
Floating-point arithmetic provides only a coarse, discrete mesh for representing real numbers.
  When complex high-dimensional geometry demands: ultra-fine resolution, huge dynamic ranges, delicate curvature distinctions, the mesh simply cannot capture it accurately → numerical failures.