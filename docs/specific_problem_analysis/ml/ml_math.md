...
- Graphical Models
	...
	- How Modern Deep Learning Uses Graphical Models
		- deep learning practitioners often use graphical-model ideas, but combine them with other design principles — resulting in models quite different from classical “PGM (probabilistic graphical model)” traditions. 
		-  Key points:
...
- Monte Carlo Methods
	...
	- Why Monte Carlo Methods are Fundamental in ML / Deep Learning
		- Many problems in ML involve computing sums or integrals over exponentially many terms (e.g. marginalizing latent variables; evaluating partition functions of undirected models; expectations under distributions). Deterministic methods often become infeasible. Monte Carlo methods provide a general and flexible way to approximate these intractable sums/integrals. 
		- Sampling-based approximations are especially valuable in high-dimensional or combinatorial spaces, where exact enumeration or analytic integration is impractical (or impossible). In those cases, Monte Carlo can give approximate answers with controllable error. 
		- Because many modern models (e.g. energy-based models, undirected graphical models, models with latent variables) define complex, high-dimensional distributions, Monte Carlo is often the only realistic way to draw samples, estimate expectations, compute gradients, or approximate likelihoods — making them central to generative modeling, approximate inference, and training in deep learning. 
	- Core Methods & Their Roles
		- Basic Monte Carlo sampling / Monte Carlo estimation
			- If you can draw independent samples x (i) from a distribution p(x), you can approximate an expectation or integral
			- This estimator is unbiased (its expectation equals the true quantity), and its variance decreases as 1/n under mild conditions — meaning more samples → more accurate. 
			- Hence, even when integrals are intractable analytically, Monte Carlo gives a way to approximate them, with quantifiable error via the law of large numbers and central limit theorem. 
		- Importance Sampling
			- When sampling directly from p(x) is hard or undesirable, you can pick another distribution q(x) that is easier to sample from, and reweight samples to get an unbiased estimate:
			- The quality (variance) of this estimator depends heavily on how well q matches the “mass” of p(x)f(x). A good choice of q — one that puts more mass where p(x)f(x) is large — reduces variance drastically. In the ideal (rarely achievable) case, the variance can be zero. 
			- However, mismatched q can lead to very high variance (or even practically useless estimates), especially in high-dimensional settings where probabilities are extremely peaked or have long tails. 
		- Markov Chain Monte Carlo (MCMC)
			- When direct sampling from a complex target distribution p(x) is infeasible, and good importance distributions q are unavailable, MCMC offers a powerful alternative. The idea is to define a Markov chain whose stationary distribution (equilibrium) is exactly p(x), then run the chain 'long enough' so that its states become (approximately) samples from p. 
			- Common techniques (e.g. Gibbs sampling) update parts of x one (or a block) at a time, sampling from their conditional distributions given the rest — which often simplifies sampling in high-dimensional structured models like energy-based models / undirected models. 
			- MCMC is widely used to estimate expectations, approximate partition functions, sample from generative models, or perform inference in probabilistic models — tasks that are otherwise intractable. 
	- Limitations & Challenges of Monte Carlo in Practice
		- While powerful, Monte Carlo methods come with significant caveats
		- Variance / inefficiency: Even for unbiased estimators, variance can be large. Reducing variance often requires many samples — which may be costly or infeasible. Importance sampling helps, but only when you can choose or approximate a good q(x). Poor q leads to slow convergence or high estimator noise.
		- Sampling from complex, high-dimensional distributions is hard: For many models (especially with energy-based undirected forms), exact sampling is impossible; MCMC may be the only option — but MCMC itself often suffers from slow mixing, meaning it explores the space very slowly, gets “stuck” in modes, or fails to reach equilibrium in reasonable time. 
		- Correlation between samples: In MCMC, successive samples are typically correlated; they are not independent. That reduces effectively how much “information” you get per sample, compared to iid sampling. You often need to discard (“thin”) samples, or run multiple chains — which is computationally expensive.
		- Difficulty assessing convergence / mixing: Because the state space is often huge (exponential in number of variables), it's in practice impossible to know exactly when a chain has “mixed” (i.e. reached its equilibrium). There are no universal guarantees for mixing time, and often practitioners rely on heuristics or manual inspection. 
		- Mode separation and energy barriers: If the target distribution has multiple modes (e.g. very different regions of high probability separated by low-probability “barriers”), MCMC (especially simple ones like Gibbs) may struggle to move between modes; the chain may stay trapped in one mode for very long — resulting in poor coverage of the true distribution. 
	- Special Considerations & Strategies: How to Make Monte Carlo Work Better
		- strategies (or helps highlight when) Monte Carlo methods can be made more practical / effective — or when they remain problematic:
			- Choice of proposal / sampling distribution matters a lot: In importance sampling, a poor q(x) leads to high variance; a well-chosen q (ideally shaped to match the “important regions” of integrand) reduces variance. While the theoretically optimal q may be intractable, clever approximations, domain knowledge, or heuristic sampling strategies can help. 
			- MCMC enhancements — tempering, multiple chains, hierarchical sampling: For multimodal distributions, methods like Parallel Tempering (or tempered transitions) can help the chain escape local modes: by sampling at higher “temperature” (flattened energy landscape), the chain moves more freely, then gradually transitions to the target distribution. 
			- Using latent variable / hierarchical representations to help mixing: For generative models where data x is generated from latent variables h, it's often easier to mix in latent space than in observation space. Deep latent-variable models can produce representations (in h-space) whose distributions are smoother and more uniform — making sampling and mixing easier in h-space, then mapping to x. 
			- Monte Carlo as “workhorse” despite imperfections: even given drawbacks — Monte Carlo (and MCMC) methods remain central and often the only feasible method in many realistic modeling situations. In many deep or undirected models, you're unlikely to get exact solutions; sampling-based approximate inference remains indispensable. 
	- Broader Significance: What This All Means for Deep Learning & Generative Modeling
		- For deep generative models (especially those that define complex probability distributions, energy-based models, undirected models, latent-variable models), Monte Carlo is often the only practical tool to do inference, sampling, approximate likelihood estimation, or expectation-based learning.
		- The limitations (variance, mixing, high-dimensional difficulties) explain — in large part — why generative modeling remains hard, especially for realistic data (images, audio, text) with complex multimodal distributions. Understanding these limitations is essential to both apply and improve these models.
		- The insights highlight what to watch out for when using Monte Carlo in practice: pick good sampling / proposal distributions; consider using advanced MCMC techniques (tempering, multiple chains, latent-space sampling); monitor mixing / convergence; be wary of high-dimensional pitfalls.
		- there is a broader trade-off in ML / deep learning: exactness vs feasibility. Monte Carlo methods embrace approximate, stochastic correctness — trading off exact answers for scalable, approximate ones. That trade-off is often necessary, and mastering it is part of the art of working with complex models.

- The Partition Function

	- What is the partition function — and why it matters
		- Many probabilistic models (particularly undirected models) define an unnormalized “score” for each possible data configuration. To make this into a valid probability distribution, you must divide by a partition function — essentially the sum or integral of that score over all possible data points. 
		- The partition function depends on the model parameters. That means if you want to learn the model by maximizing likelihood, you need to account for how changes in parameters shift the partition function — which is typically intractable for realistic, high-dimensional models. 
		- In short: the partition function is what normalizes the model, but computing it (or its gradient) tends to be computationally infeasible for many interesting models. That’s a big challenge (often a “bottleneck”) in probabilistic / generative modeling. 
	- Learning vs the Partition Function — “Positive” and “Negative” Phases
		- Because direct calculation of the partition function is intractable, learning proceeds conceptually in two “phases”:
		- Positive phase — raising the model’s score for actual data (i.e. make training data more probable under the unnormalized model).
		- Negative phase — pushing down the model’s normalized probability for everything else (i.e. make sure the whole distribution remains normalized: the partition function accounts for total mass over all states). 
		- The negative phase is the hard part, because it involves the partition function (or equivalently expectations over the model’s full distribution), which we can’t compute exactly in general. 
	- Techniques to Train / Evaluate Despite the Intractability
		- Because exact normalization is often impossible, several practical strategies — each with trade-offs — to get around or approximate the partition-function problem:
		- Use sampling / MCMC (or related Monte-Carlo methods) to approximate the “negative phase”: approximate the effect of partition-function normalization by drawing approximate samples from the model distribution instead of summing over all possible data. 
		- Use alternative (surrogate) criteria that do not require normalization. For example:
		- Pseudolikelihood — rather than modeling full joint distribution, focus on conditional distributions for parts of the data given others. Because partition constants cancel in ratios, normalization isn’t needed. 
		- Score-matching / noise-based approaches / contrastive methods — formulate objectives that depend on unnormalized scores in a way that avoids needing the partition constant. 
		- Noise-contrastive estimation (NCE) — recast density estimation as a classification problem (real data vs artificially generated “noise” data), bypassing explicit partition-function computation. 
		- When you need normalized probabilities (e.g. to evaluate likelihoods, compare models, compute log-likelihood of test data), you can use approximate partition-function estimation methods:
		- Importance sampling + annealed / tempered sampling — use a simpler “proposal” distribution (or a sequence of intermediate distributions) and reweight samples to approximate the normalization constant (or to estimate the ratio of normalization constants between two models). 
		- Bridge sampling and variants — more advanced estimators to approximate partition functions (or ratios) with reduced variance, used when direct importance sampling is too inefficient. 
	- Broader Lessons — Trade-offs, What Modelers Must Watch Out For
		- There is a fundamental trade-off between expressiveness and tractability: richer, more flexible undirected models can in principle model complex distributions — but normalizing them becomes intractable. If you build such a model, you must commit to approximate methods for training / evaluation.
		- Approximate methods come with costs and limitations: sampling-based methods depend on having good mixing / good proposal distributions; surrogate criteria may bias the model away from true likelihood optimum; partition-function estimates can have high variance or be computationally expensive.
		- Because of these difficulties, some models are designed from the start to avoid the partition function altogether (e.g. normalized models with tractable normalizing constants), or are used in ways that don’t require computing full probabilities (e.g. feature extraction, representation learning, energy-based heuristics) — a design decision with deep implications for what the model can or cannot do. 
		- When you do need actual densities (for sampling, model comparison, likelihood evaluation), you must explicitly deal with the partition-function challenge: choose an estimation method (importance sampling, AIS, etc.), and accept the trade-offs (computation, estimation error, complexity). 
	- Why This Matters: For Anyone Working with Generative / Probabilistic Models
		- The partition-function issue is at the core of why many generative / probabilistic models are hard to train, evaluate, or use in practice. Understanding this provides clarity on what problems are real and what aspects are just math formalities — and helps set realistic expectations.
		- practical strategies (sampling, surrogate criteria, estimation methods) enable building and using models despite these fundamental problems. That makes probabilistic modeling more accessible, but also forces modelers to understand and manage trade-offs.
		- It shows that probabilistic modeling isn’t just about picking a flexible model — it's also about thinking carefully about how you will normalize, train, sample, or evaluate. Choices made at model design time can drastically affect feasibility, performance, and ultimate usefulness.
		- For modern deep generative modeling: this analysis helps explain why some models are built to avoid partition-function issues entirely (e.g. using architectures with tractable normalization), and why others rely heavily on approximations and heuristics.

- Approximate Inference

	- What “Inference” means (and why it’s hard in deep / latent-variable models)
		- In probabilistic models used in deep learning, we often have visible variables (observed data) and latent (hidden) variables. Inference means computing the distribution of the latent variables given observed data — e.g. p(h∣v) — or expectations under that distribution. 
		- For many “simple” models (e.g. certain one-hidden-layer, specially structured ones), inference is tractable. But for many interesting and expressive models — especially those with multiple layers of latent variables, or with complex dependencies among latent variables — the posterior becomes intractable (cannot compute exactly or efficiently). 
		- This intractability is a fundamental bottleneck in training and using deep probabilistic generative models: without inference, you cannot reliably evaluate likelihoods, learn model parameters, or sample from the model. 
	- Approximate Inference: Trading Exactness for Tractability
		- Because exact inference is often impossible, the chapter describes a set of methods to approximate inference — accepting an approximation to make things computationally feasible. Key ideas:
		- Treating inference as optimization: Instead of trying to compute the exact posterior, one defines an optimization problem whose solution is (ideally) a good approximation to the posterior. Concretely, you define a “proxy” distribution q(h∣v), and optimize it so that it approximates the true posterior as closely as possible.
		- This yields a lower bound on the log-likelihood of data (often called the “evidence lower bound,” or ELBO). By optimizing the bound, you get a model that — though approximate — is trainable, and inference becomes tractable. 
		- The quality of approximation depends on how expressive the family of approximate distributions q is (i.e. how restrictive or flexible), and how well the optimization is done. There's a trade-off: simpler q → cheaper inference but cruder approximation; more complex q → better approximation but heavier computation.
	- Common Inference Methods & Their Tradeoffs
		- The chapter surveys several common approaches to approximate inference — each appropriate in different contexts depending on model complexity, computational budget, and application needs: 
		- EM (Expectation-Maximization): A classic algorithm that alternates between inferring latent variables (E-step) and updating model parameters (M-step). Useful when posterior inference is tractable, but becomes difficult when posterior is complex. 
		- MAP inference (Maximum a Posteriori): Instead of inferring a full distribution over latent variables, simply pick the most likely latent configuration given data. This is often used with models that have intractable posteriors (e.g. sparse coding) — trading uncertainty for a single, point estimate. 
		- Variational inference / Variational learning: Approximate the true posterior with a simpler, tractable distribution family (e.g. assuming latent variables are independent) and optimize to get the “best” approximate posterior. This is among the most widely used approaches for latent-variable deep models.
		- Within this, a common simplified variant is the mean-field approximation, which assumes the latent variables factorize (i.e. no dependencies among them), making computations tractable. 
		- More expressive variants are possible (“structured variational inference”) — if one is willing to pay computational cost for better approximations. 
		- Learned / amortized inference: Rather than doing optimization per data point at inference time, one can train a dedicated network (an “inference network” / “recognition model”) that takes observed data as input and predicts approximate latent distributions as output. This is the idea behind modern latent-variable models in deep learning, and it makes inference fast and scalable. 
		- Because this inference network is learned jointly with the generative model, the model can adapt so that its posterior becomes more amenable to the inference approximation — in effect, the learning process encourages the model to make inference easier. 
	- Deeper Implication: Learning Shapes the Posterior to Match What Inference Can Handle
		- One especially important insight: when you train a model using approximate inference, the model tends to adapt so that its real (true) posterior becomes more similar to whatever family/factorization your approximate inference method assumes. That is, the act of approximate inference during training “pulls” the model toward regions where inference is easier. 
		- On the one hand, this “self-fulfilling” effect is useful: it makes inference and sampling more tractable.
		- On the other hand, it risks biasing the model — the model may avoid truly complex (multi-modal, highly dependent) latent structures, because those would break the assumptions of the inference algorithm. 
		- This trade-off means that approximate inference is not a “free lunch”: you get scalability and practicality at the cost of limiting what your model can represent (or at least what latent structures it’s willing to adopt). 
	- Why This Matters for Modern Deep Generative Models
		- Many of the generative / latent-variable models used in deep learning — e.g. autoencoders with probabilistic latent spaces, deep belief nets, variational autoencoders — rely on approximate inference under the hood. The techniques from this chapter provide the theoretical foundation to understand why these models work and how inference is done.
		- The shift to learned inference (inference networks) is particularly influential: it enables scaling to large datasets and high-dimensional data (images, audio, text), where per-sample optimization would be too slow.
		- Understanding the trade-offs — approximation vs fidelity, expressiveness vs tractability — helps in designing models and choosing inference methods intentionally (not blindly).

- Deep Generative Models

	- What “Generative Models” mean (in context) — and why they matter
		- A “generative model” seeks to learn (or represent) a full probability distribution over data (not just classify or predict). Once learned, such a model can be used to sample new data, perform density estimation, or support inference tasks (e.g. missing data, conditional generation, structured output, etc.).
		- Such models are powerful because they aim to capture the structure, variation, and regularities in the data — not just decision boundaries. That makes them useful for unsupervised learning, data generation, semi-supervised tasks, data augmentation, etc.
		- There are many kinds of generative models — some allow explicit evaluation of densities (so you can compute likelihoods), others don’t but support sampling. Some are naturally expressed as probabilistic (graphical) models, others are neural networks (or hybrids). 
		- Because real-world data (images, text, audio) are high-dimensional and complex, generative models are often more difficult than discriminative models — but also more broadly useful.
	- Key Classes / Families of Deep Generative Models
		- The chapter reviews many different architectures and approaches. Some of the most important:
		- Energy-based / undirected models — e.g. Boltzmann Machine / variants. These define a probability distribution via an energy function (unnormalized “score”) over data (or data + latent variables), then normalize with a partition function. This formalism is very general and can — in principle — approximate very complex distributions. 
		- Partially directed / hybrid models — e.g. Deep Belief Network (DBN): these mix directed and undirected layers, providing a compromise between expressiveness and tractability. Historically, DBNs were among the first deep architectures to show promise for generative modeling, helping to usher in the “deep-learning renaissance.” 
		- Fully directed generative models with latent variables — these define a generative process: sample a latent code, then transform through a (possibly deep) decoder network to generate data. A prominent modern example is Variational Autoencoder (VAE). In these models, both sampling and approximate inference (via an encoder / inference network) are possible, often making training scalable via gradient-based optimization. 
		- Fully visible, autoregressive / conditional models — these do not use latent variables; instead they decompose the joint distribution over data dimensions using the chain rule, modeling each conditional with a neural network. This approach can provide exact likelihoods & sampling (e.g. sequential sampling), and can be simpler conceptually than latent-variable models. 
		- “Generator-net” models that directly map simple random noise to data — rather than defining an explicit density, these generate samples by feeding random input into a network. Although classical probabilistic semantics (density evaluation) may be lost or approximated, this approach is often efficient, flexible, and widely used in practice. 
	- Tradeoffs, Challenges & What Generative Modeling Requires
		- The chapter emphasizes that generative modeling — especially in high dimensions — comes with fundamental trade-offs and challenges. Some of the most important:
		- Tractability vs. Expressiveness: Highly expressive models (e.g. deep energy-based models, dense directed/deep networks) can in principle represent very complex distributions — but inference (computing probabilities, sampling, latent-variable inference) often becomes intractable. 
		- Need for approximations / surrogate methods: For many such models, one cannot compute exact likelihoods or exact inference. Thus, approximate inference techniques (e.g. variational inference, sampling, learned inference networks) are critical. 
		- Training difficulty / instability: Models that generate samples by directly mapping noise to data — or that rely on adversarial losses or implicit densities — often involve delicate training regimes. Achieving stable learning, avoiding degeneracy, ensuring diversity (not just memorizing training data), and balancing quality vs variation are nontrivial. 
		- Evaluation is hard: For many generative models, especially implicit ones (that don’t give tractable likelihoods), evaluating how good the model is remains challenging. What does it mean for generated samples to be “good”? Do they cover the full diversity of real data? Do they assign reasonable “probability mass” to realistic data? These are open and hard questions. 
		- Modeling vs generation goals differ: A model optimized for likelihood (or density estimation) may not produce the best samples; a model optimized for sample realism may not assign accurate densities. Depending on application — sampling, density estimation, representation learning — one may prefer different model classes. 
	- Why Deep Generative Models Matter (and What They Offer)
		- Unsupervised learning & representation learning: Generative models don’t require labels, so they can leverage large unlabeled datasets to learn representations, latent structure, and distributions — often revealing underlying causes / factors of variation.
		- Data generation / augmentation: Once a generative model is trained, you can sample novel data — useful for data augmentation, simulation, creative generation (images, audio, text), and synthetic data for rare or expensive cases.
		- Flexible modeling of complex data: Deep generative models can potentially approximate distributions over very complex, high-dimensional data (images, audio, text), capturing multimodality, nonlinearity, structure.
		- Probabilistic reasoning & uncertainty modeling: Because they aim to model distributions, generative models naturally support uncertainty estimation, missing data imputation, conditional generation, and more — going beyond deterministic prediction or classification.
		- Foundations for advanced generative systems: Many modern state-of-the-art generative systems (VAEs, GANs, autoregressive models, flows, etc.) trace conceptual roots to the classes of models described in the chapter. Understanding them gives a theoretical grounding behind what makes these models work, what trade-offs they embody, and where limitations lie.