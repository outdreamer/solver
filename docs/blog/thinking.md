...
- maximizing relevant thinking types: thinking types can be 'ways to analyze' or 'ways to connect structures' that can 'solve any problem' (like 'interface/requirements/structure-based thinking'), so a 'thinking type on a graph' could be a 'way to interpret/query the graph'
- maximizing relevant intelligence types: maximizing relevant intelligence types like 'intelligence functions' like 'derivation/logic/prediction' is similarly useful

Optimal interactions include:
- a 'meta-mind' is an example of an optimal interaction between minds, which would optimally allow 'running any thinking type and support all intelligence types/functions' on any or a subset of the minds, and optimally organize its minds in a graph that optimizes the mind interactions
