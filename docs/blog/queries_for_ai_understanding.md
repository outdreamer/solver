# Queries to increase AI understanding

It's useful to identify queries to identify/generate useful understanding structures like graphs/insights/concepts.


## Example queries to increase understanding

Example queries: 
- summarize recent progress in this field (a field like AI optimization), up to the last 3 iterations of a workflow like 'generation/optimization'
- identify new general similarities/patterns that can be differentiated into new concepts and define the concepts and generate a few examples of an application of each concept
- fit this new change into a set of relevant graphs, and explain how the graphs are optimized now (like being 'more complete' now)
- identify the most different structures in this field, compared to this other similar/related field
- identify the unsolved problems in this field, and 'probable solutions to those problems', by applying imminent/existing solutions
- identify complex/suboptimal structures like 'specific exceptions' that arent trivially/relevantly understood yet and identify possible connections between these suboptimal structures like 'causal connections'


## Understanding/insight query types

These queries are intended to 'identify insights/other understanding structures without explicitly requesting insights', since the AI may not already be able to identify insights in that field, which is applying these queries as 'alternate definitions of insight/understanding' that can help the AI identify insights when it doesn't already know them.

Other than 'alternate definitions of insight/understanding structures', other query types which are useful for generating understanding structures include:
- abstractions of insight/understanding like 'connections' as an opposing method of 'specification/definition'
- alternate workflows to 'generate understanding inputs/requirements' or 'avoid understanding errors like ambiguities/confusion'
- alternate variables/differentiators/filters of understanding structures, like to differentiate 'more relevant understanding structures'